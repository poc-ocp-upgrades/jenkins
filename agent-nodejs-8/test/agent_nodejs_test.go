...
var _ = BeforeSuite(func() {
	...
	if imageName == "" {
		imageName = "openshift/jenkins-agent-nodejs-8-centos7-candidate"
	}
})
var _ = Describe("NodeJS agent testing", func() {
	...
	It("should contain a runnable npm", func() {
		var err error
		id, err = dockercli.ContainerCreate(&container.Config{Image: imageName, Entrypoint: []string{"/bin/bash", "-c", "npm --version"}, Tty: true}, nil)
		Expect(err).NotTo(HaveOccurred())
		err = dockercli.ContainerStart(id)
		Expect(err).NotTo(HaveOccurred())
		code, err := dockercli.ContainerWait(id)
	...
	It("should contain a runnable node", func() {
		var err error
		id, err = dockercli.ContainerCreate(&container.Config{Image: imageName, Entrypoint: []string{"/bin/bash", "-c", "node --version"}, Tty: true}, nil)
		Expect(err).NotTo(HaveOccurred())
		err = dockercli.ContainerStart(id)
		Expect(err).NotTo(HaveOccurred())
		code, err := dockercli.ContainerWait(id)
