...
var _ = Describe("Jenkins testing (v2)", func() {
	...
	basePlugins := []string{"ace-editor", "branch-api", "credentials", "durable-task", "cloudbees-folder", "git", "git-server", "git-client", "handlebars", "momentjs", "plain-credentials", "scm-api", "script-security", "structs"}
	additionalPlugins := []string{"ansicolor", "greenballs"}
	...
	It("should pass a smoke test after an s2i build", func() {
		...
		cmd := exec.Cmd{Path: s2i, Args: []string{s2i, "build", "--pull-policy=never", "--loglevel=5", "testdata/s2i", imageName, destImage}, Stdout: GinkgoWriter, Stderr: GinkgoWriter}
		err = cmd.Run()
		Expect(err).NotTo(HaveOccurred())
		cancel()
